...
class RealSphincs:
    ...
    def __init__(self, security_level: int = 3):
        ...
        if security_level not in sphincs_names:
            raise ValueError(f"Unsupported security level: {security_level}. Use 1, 3, or 5.")

        self.algorithm_name = sphincs_names[security_level]
        self.signer = oqs.Signature(self.algorithm_name)

        self.private_key = None
        self.public_key = None

        print(f"SPHINCS+ initialized: {self.algorithm_name}")
...
class LegalDatasetSecuritySystem:
    ...
    def __init__(self, dataset_path: str, security_level: int = 3):
        self.dataset_path = dataset_path
        self.security_level = security_level
        self.sphincs = RealSphincs(security_level)

        self.output_base = "secured_dataset_sphincs"
    ...
    def sign_all_documents(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        print("\nSIGNING ALL DOCUMENTS WITH SPHINCS+")
        print("=" * 70)
        ...
        public_key_file = os.path.join(self.output_base, "public_key_sphincs.bin")
        ...
        for _ in range(WARMUP_ITERATIONS):
            warmup_sig = self.sphincs.sign_document("warmup placeholder document content")
            self.sphincs.verify_signature("warmup placeholder document content", warmup_sig, public_key)
        ...
        for idx, doc in enumerate(documents, 1):
            try:
                ...
                signature_data = self.sphincs.sign_document(doc["content"], metadata)
    ...
    def verify_all_signatures(self, documents: List[Dict[str, Any]],
        ...
        for idx, signed_doc in enumerate(signed_summary["signed_documents_list"], 1):
            try:
                ...
                verification = self.sphincs.verify_signature(
    ...
    def generate_report(self, documents: List[Dict[str, Any]],
        ...
        metrics_data = {
            "metadata": {
                ...
                "security_level": self.security_level,
                "algorithm": self.sphincs.algorithm_name
            },
            "dataset_summary": {
            ...
            "key_sizes": {
                "public_key_bytes": len(self.sphincs.public_key) if self.sphincs.public_key else 0,
                "private_key_bytes": len(self.sphincs.private_key) if self.sphincs.private_key else 0
            },
            "signing_metrics": {
        ...
        json_filename = f"security_metrics_sphincs_{timestamp}.json"
...
def main():
    ...
    print("Algorithm: SPHINCS+-SHA2-192f-simple (NIST Level 3)")
    print("=" * 70)
    ...
    print(f"Security level : NIST Level {security_level} (SPHINCS+-SHA2-192f-simple)")
