...
class RealDilithium:
    ...
    def __init__(self, security_level: int = 3):
        ...
        if security_level not in ml_dsa_names:
            raise ValueError(f"Unsupported security level: {security_level}. Use 2, 3, or 5.")

        self.algorithm_name = ml_dsa_names[security_level]
        self.signer = oqs.Signature(self.algorithm_name)

        self.private_key = None
        self.public_key = None

        print(f"ML-DSA initialized: {self.algorithm_name}")
...
class LegalDatasetSecuritySystem:
    ...
    def __init__(self, dataset_path: str, security_level: int = 3):
        ...
        self.security_level = security_level
        self.dilithium = RealDilithium(security_level)

        self.output_base = "secured_dataset_mldsa"
    ...
    def sign_all_documents(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        print("\nSIGNING ALL DOCUMENTS WITH ML-DSA")
        print("=" * 70)

        print("Generating ML-DSA keypair...")
        private_key, public_key = self.dilithium.generate_keypair()

        public_key_file = os.path.join(self.output_base, "public_key_mldsa.bin")
        ...
        for _ in range(WARMUP_ITERATIONS):
            warmup_sig = self.dilithium.sign_document("warmup placeholder document content")
            self.dilithium.verify_signature("warmup placeholder document content", warmup_sig, public_key)
        ...
        for idx, doc in enumerate(documents, 1):
            try:
                ...
                signature_data = self.dilithium.sign_document(doc["content"], metadata)
    ...
    def verify_all_signatures(self, documents: List[Dict[str, Any]],
        ...
        for idx, signed_doc in enumerate(signed_summary["signed_documents_list"], 1):
            try:
                ...
                verification = self.dilithium.verify_signature(
    ...
    def generate_report(self, documents: List[Dict[str, Any]],
        ...
        metrics_data = {
            "metadata": {
                ...
                "security_level": self.security_level,
                "algorithm": self.dilithium.algorithm_name
            },
            "dataset_summary": {
            ...
            "key_sizes": {
                "public_key_bytes": len(self.dilithium.public_key) if self.dilithium.public_key else 0,
                "private_key_bytes": len(self.dilithium.private_key) if self.dilithium.private_key else 0
            },
            "signing_metrics": {
        ...
        json_filename = f"security_metrics_mldsa_{timestamp}.json"
...
def main():
    ...
    print(f"Security level   : NIST Level {security_level} (ML-DSA-65)")
